...
class Settings(BaseModel):
    ...
    data_dir: Path = Field(default=Path("/root/bot/data"), alias="DATA_DIR")
    log_dir: Path = Field(default=Path("/root/bot/logs"), alias="LOG_DIR")
    db_path: Path = Field(default=Path("/root/bot/data/bot.sqlite3"), alias="DB_PATH")
...
def load_settings() -> Settings:
    ...
    if missing:
        raise RuntimeError(
            f"Missing required env vars: {', '.join(missing)}. Create /root/bot/app/.env"
        )
